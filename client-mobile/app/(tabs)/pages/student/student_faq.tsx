    ...
    >
      <View style={styles.accordionHeaderRow}>
        ...
        <View style={{ transform: [{ rotate: expanded ? '180deg' : '0deg' }] }}>
          <ChevronDown size={18} color={expanded ? theme.accent : theme.subtext} />
        </View>
      </View>
      {expanded && (
...
export default function StudentFaqScreen() {
  ...
  // FAQ handler doesn't show a toast on refresh, so this doesn't either). ──
  useEffect(() => {
  ...
  const handleNavPress = (key: string) => {
    setMenuOpen(false);
    if (key === 'dashboard') {
      router.push('/pages/student/student_dashboard');
      return;
    }
