...
// "Reposted" from then on. Matches the student page's convention.
...
export default function ProfessorAnnouncementScreen() {
  ...
  // Guards against out-of-order responses (mirrors student_announcement.tsx).
  ...
  // ── Live updates: refetch + notify when an admin posts/edits/removes a
  // faculty-audience announcement (mirrors student_announcement.tsx). ──
  useEffect(() => {
  ...
  const renderCard = (announcement: Announcement) => {
    const style = announcement.isPinned ? PINNED_STYLE : DEFAULT_CARD_STYLE;
    return (
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {error && (
            <View style={styles.emptyCard}>
              ...
              <Pressable style={styles.collegeFilterBtn} onPress={() => fetchAnnouncements(1)}>
            ...
            {announcements.length === 0 ? (
              <View style={styles.emptyCard}>
                ...
                <Text style={styles.emptyDescription}>
                  Announcements for faculty in your department will appear here.
                </Text>
              </View>
            ) : (
              <>
...
// Copied verbatim from professor_dashboard.tsx's own palette (not
// student_announcement.tsx's) so this screen stays aligned with its 7
// sibling professor screens' palette source -- the values are identical
// either way (both modules already share the same green theme).
...
function createStyles(theme: ThemePalette) {
  return StyleSheet.create({
    ...
    // Confirm logout modal (shared card look)
    logoutOverlay: {
      flex: 1,
      alignItems: 'center',
      justifyContent: 'center',
